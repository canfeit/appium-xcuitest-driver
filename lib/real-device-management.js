...
async function installToRealDevice (driver, device, app, bundleId, noReset = true) {
  if (!device.udid || !app) {
  ...
  // testwa: timer here for installing the app
  let myDate = new Date();
  let startTime = myDate.getTime();
  await device.install(app);
  myDate = new Date();
  let endTime = myDate.getTime();
  driver.installAppTime = endTime - startTime;
